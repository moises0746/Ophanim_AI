...
Ophanim_AI/
|-- apps/                                  # First-party
|   `-- desktop/                           # Runnable Tauri/React Assistant runtime
...
|       |   |-- api/                       # Implemented routes, including Assistant SSE delivery
|       |   |-- adapters/                  # Provider, gate, and event-delivery adapters
...
- `services/ophanim-core/ophanim/` contains the FastAPI runtime, Core domain/application/ports, authenticated Assistant SSE and chat/model delivery, loopback LM Studio chat, and governed OpenAI/Gemini/Anthropic adapters with execution-time secrets.
- `apps/desktop/` contains the runnable Tauri/React Assistant. Its Rust bridge owns the ephemeral local Core credential and exposes only bounded model/chat/event commands; React presents conversation, routing controls, and authoritative event-derived state.
...
- Voice capture/playback, durable chat history, production credential-store integration, and signed Desktop packaging remain deferred. The implemented Desktop continues to require authoritative Core events for state/activity and cannot execute providers or tools directly.
