...
Ophanim_AI/
|-- apps/                                  # First-party
|   `-- desktop/                           # Implemented Tauri/React Assistant shell
...
- `services/ophanim-core/ophanim/` contains the current FastAPI runtime, configuration, provider adapters, preserved experimental browser implementation, Core domain/application/port ownership, the R1-12 authenticated default-deny Assistant SSE delivery boundary, and R1-06A governed OpenAI/Gemini/Anthropic text-model adapters with execution-time secret resolution. AAO-001 adds the deterministic autonomous agent orchestration foundation: workflow state machine, Planner/Developer/QA/Reviewer/Orchestrator role model, provider-agnostic agent port, quality-gate abstractions, in-memory workflow repository/event store, gate runners, and the `WorkflowOrchestrator` application service.
- `apps/desktop/` contains the R1-11 Tauri/React Assistant shell and R1-12 typed Core-event client/projection. It presents event-derived state/activity/approval information and remains non-authoritative.
